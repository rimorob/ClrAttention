...
for (cfg in list(list("none","fd"), list("rank","fd"), list("rank",8))) {
  r <- sapply(1:6, one, tr = cfg[[1]], bins = cfg[[2]]); m <- rowMeans(r)
  cat(sprintf("%-5s %-3s AUPR %.3f  FDR TP %.1f FP %.1f | HC TP %.1f FP %.1f\n", cfg[[1]], cfg[[2]], m["aupr"], m["tp_fdr"], m["fp_fdr"], m["tp_hc"], m["fp_hc"])) }
