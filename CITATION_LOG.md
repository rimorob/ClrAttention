...
**Addendum (Linux workstation, hyper-threading).**  `tools/setup_linux.sh` sets up the dual-socket Xeon (Ubuntu); it was tested end to end on Ubuntu 24.04.  Workers are placed one per physical core in every case.  At about 3.2 GB per chips worker, 128 GB supports about 28 workers, so RAM rather than cores limits the worker count there, and a second worker per core is not possible for the full-size runs.  Hyper-threading is therefore offered at the OpenMP level only:  with `CLR_HT=1`, each worker's MI kernel runs on both hyper-threads of its core.  `tools/bench_ht.sh` times one full round of chips draws with and without it, and checks that the numbers are identical.  The default stays off until that benchmark shows a gain.
